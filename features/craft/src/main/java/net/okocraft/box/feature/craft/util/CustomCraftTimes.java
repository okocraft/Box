...
// TODO: Share the code with TransactionAmountHolder
public final class CustomCraftTimes {

    private static final Map<UUID, Unit> UNIT_MAP = new HashMap<>();
    private static final Map<UUID, AtomicInteger> CRAFT_TIMES_MAP = new HashMap<>();

    public static int getAmount(@NotNull Player player) {
        return Optional.ofNullable(CRAFT_TIMES_MAP.get(player.getUniqueId()))
    ...
    private static @NotNull AtomicInteger getOrCreate(@NotNull Player player) {
        return CRAFT_TIMES_MAP.computeIfAbsent(player.getUniqueId(), k -> new AtomicInteger(1));
    }
