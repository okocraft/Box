...
final class AdditionalRecipes {
    ...
    static void addFireworkRocketRecipes(@NotNull Processor processor) {
        ...
        BoxItem paper = itemManager.getBoxItem("PAPER").orElse(null);
        BoxItem gunpowder = itemManager.getBoxItem("GUNPOWDER").orElse(null);
        ...
        List<BoxIngredientItem> gunpowderIngredientItems = List.of(new BoxIngredientItem(gunpowder, 1));
        List<BoxIngredientItem> paperIngredientItems = List.of(new BoxIngredientItem(paper, 1));

        IntStream.of(2, 3).forEach(power -> {
            BoxItem firework = itemManager.getBoxItem(ItemType.FIREWORK_ROCKET.createItemStack(1, meta -> meta.setPower(power))).orElse(null);
            if (firework == null) {
                BoxLogger.logger().warn("Cannot create firework rocket recipes because firework (power {}) is not found", power);
                return;
            }

            List<IngredientHolder> ingredients = new ArrayList<>(power + 1);
