...
class NotificationFactoryTest {
    ...
    private static final Component EXPECTED_INCREASED_NOTIFICATION = text().append(ITEM.getDisplayName(), COMMON_PARTS_1, text(10, WHITE), COMMON_PARTS_2, text("+5", AQUA), COMMON_PARTS_3).asComponent();
    private static final Component EXPECTED_DECREASED_NOTIFICATION = text().append(ITEM.getDisplayName(), COMMON_PARTS_1, text(10, WHITE), COMMON_PARTS_2, text("-5", RED), COMMON_PARTS_3).asComponent();
    ...
    void testPlus1AndMinus1() {
        Assertions.assertEquals(
            text().append(ITEM.getDisplayName(), COMMON_PARTS_1, text(1, WHITE), PLUS_1).asComponent(),
            new NotificationFactory(ITEM, 1).increments(1).createNotification(ITEM.getDisplayName())
        );

        Assertions.assertEquals(
            text().append(ITEM.getDisplayName(), COMMON_PARTS_1, text(0, WHITE), MINUS_1).asComponent(),
