...
abstract class AbstractStockHolder implements StockHolder {

    private final StockEventCaller eventCaller;
    private final Int2ObjectOpenHashMap<Stock> stockMap;
    private final StampedLock lock = new StampedLock();

    protected AbstractStockHolder(@NotNull StockEventCaller eventCaller) {
        this(eventCaller, new Int2ObjectOpenHashMap<>());
    }

    protected AbstractStockHolder(@NotNull StockEventCaller eventCaller, @NotNull Int2ObjectOpenHashMap<Stock> stockMap) {
    ...
    protected final @Nullable Stock getStockOrNull(int internalId) {
        Stock stock;
    ...
    protected final @Nullable Stock getStockOrPutNewStock(int internalId, int initialValue) {
        long stamp = this.lock.writeLock();
    ...
    protected final @NotNull Collection<BoxItem> getStockedItems(@NotNull IntFunction<@Nullable BoxItem> idToBoxItem) {
        ...
        try {
            ...
            for (var entry : entrySet) {
                ...
                var item = idToBoxItem.apply(entry.getIntKey());
    ...
    protected void writeStockMap(@NotNull StringBuilder builder) {
        long stamp = this.lock.readLock();
