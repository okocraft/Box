...
import net.okocraft.box.storage.api.registry.StorageType;
...
public class Config {
    ...
    public @NotNull Storage loadAndCreateStorage(@NotNull StorageRegistry storageRegistry) throws IOException {
        ...
        Storage storage = this.createStorageFromSection(loaded, storageRegistry);
    ...
    private void loadCoreSetting(@NotNull MapNode source, boolean applyDefaults) {
        MapNode coreSection = source.getOrCreateMap("core");
        RecordDeserializer<CoreSetting> coreDeserializer = RecordDeserializer.create(CoreSetting.class, KeyGenerator.CAMEL_TO_KEBAB);
        ...
        if (applyDefaults) {
            ...
            RecordSerializer<Record> serializer = RecordSerializer.create(KeyGenerator.CAMEL_TO_KEBAB);
    ...
    private @NotNull Storage createStorageFromSection(@NotNull MapNode source, @NotNull StorageRegistry registry) {
        MapNode storageSection = source.getOrCreateMap("storage");
        ...
        String storageType = storageSection.getString("type").toLowerCase(Locale.ENGLISH);
        ...
        for (StorageType<?> type : registry.getEntries()) {
            String name = type.name().toLowerCase(Locale.ENGLISH);
            MapNode defaultSetting = serializer.serializeDefault(type.settingClass());
        ...
        if (storage == null) {
            StorageType<?> defaultEntry = registry.getDefault();
    ...
    private static void applyDefaults(@NotNull MapNode source, @NotNull MapNode target) {
        for (Map.Entry<Object, Node<?>> defaultEntry : source.value().entrySet()) {
